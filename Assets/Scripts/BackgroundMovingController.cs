...
public class BackgroundMovingController : MonoBehaviour
{
	...
	{
		if (usedCamera == null)
			usedCamera = Camera.main.transform;
		materials = new Material[backgroundLayers.Length];              // create a new array of the layers' materials
		for (int i = 0; i < materials.Length; i++)
		{
			if (backgroundLayers[i].name == "Nebula Aqua-Pink")
			{
				materials[i] = backgroundLayers[i].layerObject.GetComponent<MeshRenderer>().materials[1];
